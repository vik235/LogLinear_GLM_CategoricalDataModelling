...
model killed/total=dose/dist=bin link=logit residuals;
title 'Logistic Regression using GENMOD';run;

proc logistic data=beetle; model killed/total=dose/lackfit influence aggregate scale=none covb residuals;
...
title 'Output Variables from LOGISTIC';run;

proc logistic data=beetle; model killed/total=/lackfit aggregate scale=none covb;
...
proc logistic data=beetle; model killed/total=dose dose*dose/lackfit influence covb;
